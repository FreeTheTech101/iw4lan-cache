...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if ( !GetDvarInt( #"scr_antilag_spawn_fix" ) )
	{
		if ( self antilag_spawn_kill( sMeansOfDeath, psOffsetTime ) )
		{
			return;
		}
	}
...
// Returns true if this damage was dealt as the player spawned but from an anti-lag rewind origin that isn't valid.
// This is to fix a specific problem with the new anti-lag rewind code introduced in TU6 (CL 815009).
// Remove this work-around once a proper fix has been implemented. The bug is DT 75458.
antilag_spawn_kill( mod, offsetTime )
{
	not_antilag_damage = "MOD_DROWN MOD_GAS MOD_SUICIDE MOD_TRIGGER_HURT MOD_UNKNOWN MOD_CRUSH MOD_TELEFRAG MOD_FALLING";

	if ( !IsDefined( self.antilag_spawn_time ) )
	{
		return false;
	}

	if ( GetTime() - offsetTime > self.antilag_spawn_time )
	{
		return false;
	}

	if ( !IsDefined( self.antilag_spawn_origin ) )
	{
		return false;
	}

	if ( DistanceSquared( self.antilag_spawn_origin, self.origin ) < 64 * 64 )
	{
		return false;
	}
			
	if ( IsSubStr( mod, not_antilag_damage ) )
	{
		return false;
	}

	//println( "^^^ denying anti-lag damage: offsetTime: " + offSetTime + " level time: " + GetTime() + " spawn time: " + self.spawnTime );
	return true;
}
...
#/
	...
	if( isPlayer( attacker ) )
	{
		...
		else
		{
			...
			{
				attacker thread  maps\mp\gametypes\_globallogic_score::giveKillXP( sMeansOfDeath, sWeapon, self );
				self maps\mp\gametypes\_copycat::copycat_clone_loadout( attacker );

				attacker  maps\mp\gametypes\_globallogic_score::incPersStat( "kills", 1, true, true );
				attacker.kills = attacker  maps\mp\gametypes\_globallogic_score::getPersStat( "kills" );
				attacker  maps\mp\gametypes\_globallogic_score::updatePersRatio( "kdratio", "kills", "deaths" );
				...
				// S. Madigan: Moved these two lines to _medals.gsc.  cur_death_streak was being reset
				//				before checking medals like Comeback, causing the medal to never work.
