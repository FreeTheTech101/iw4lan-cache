...
self endon("death");
    ...
    self notifyOnPlayerCommand("1bot", "+actionslot 2");
	for(;;)
		{
			self waittill("1bot");
...
initIndividualBot()
{
    ...
    wait 0.5;
    self notify("menuresponse", "changeclass", "class" + randomInt( 1 ));
}
...
FixQuickScopeExploit()
	...
	self notifyOnPlayerCommand("[{toggleads}]", "toggleads");
	
	while(1) {
	
		self waittill("[{toggleads}]");
...
AntiCamp()
{
	...
	for(;;)
	{
			...
			self.after = self getorigin();
				...
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + " ^7будет убит за кемнинг ч-з ^15.");
				wait 1;
				self.after = self getorigin();
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + " ^7будет убит за кемнинг ч-з ^14.");
				wait 1;
				self.after = self getorigin();
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + " ^7будет убит за кемнинг ч-з ^13.");
				wait 1;
				self.after = self getorigin();
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + " ^7будет убит за кемнинг ч-з ^12.");
				wait 1;
				self.after = self getorigin();
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + " ^7будет убит за кемнинг ч-з ^11.");
				wait 1;
				self.after = self getorigin();
				if( ( distance(self.before, self.after) < 40) ) {
				announcement("^1" + (self.name) + "^1 убит за кемпинг!");
				self suicide();
								}
							}
						}
...
onJoinedTeam()
{
	...
	for(;;)
	{
		...
		self thread doAmmo();
		self thread maps\mp\gametypes\_hud_message::hintMessage("^3Интервейшн");
		//self thread maps\mp\gametypes\_hud_message::hintMessage("^7Автор ^9rimo^17^17^37 ^1& ^7редактор VAS ^5Rai^3den");
		//self thread maps\mp\gametypes\_hud_message::hintMessage("some thing are by ChRoNiiCz");
	}
}
...
onJoinedSpectators()
{
	...
	for(;;)
	{
		...
		self thread removeRankHUD();
		//self thread maps\mp\gametypes\_hud_message::hintMessage("If you see a ^1cheater ^2report him to ^5admin");
		//self thread maps\mp\gametypes\_hud_message::hintMessage("^1donґt glitch");
		//self thread maps\mp\gametypes\_hud_message::hintMessage("^9future ^3i^1S^3nipe");
	}
}
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		//self thread FixQuickScopeExploit();
...
jetpack()
{
        ...
        else JETPACKTXT settext("Полёт [ ^3F ^7]");
